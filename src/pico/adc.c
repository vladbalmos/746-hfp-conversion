...
static int dma_chan;
static sample_rate_t dma_sample_rate;
...
void adc_start_streaming() {
    ...
    printf("Writing data\n");
    for (int i = 0; i < max; i++) {
        spi_write16_blocking(spi1, &sinewave_16000[i], 1);
    }
    printf("Wrote data\n");
    // uint16_t dma_timer_num;
    // uint16_t dma_timer_denom;

    // if (dma_sample_rate == SAMPLE_RATE_16KHZ) {
    //     dma_timer_num = 8;
    //     dma_timer_denom = 62500;
    // } else if (dma_sample_rate == SAMPLE_RATE_8KHZ) {
    //     dma_timer_num = 4;
    //     dma_timer_denom = 62500;
    // }
    ...
    // printf("Transfering data at %f hz %d %d %d %d\n", actual_sample_rate, cpu_clock_freq_hz, dma_timer_num, dma_timer_denom, dma_sample_rate);
    
    // dma_timer_set_fraction(ADC_DMA_TIMER, dma_timer_num, dma_timer_denom);
    // dma_channel_set_trans_count(dma_chan, 2, false);
    // dma_channel_set_read_addr(dma_chan, sinewave_16000, true);
}
